...
def create_app():
    ...
    @login_required
    def dashboard():
        # 1. Sincronización (Asegúrate de que solo haya UN Insert)
        ...
        # 2. Consultas
        niveles_query = db.session.execute(text("SELECT a.nivel, COUNT(*) as total FROM dw.dim_miembros m JOIN atletas a ON m.miembro_id = a.miembro_id GROUP BY a.nivel")).mappings().all()
        roles_query = db.session.execute(text("SELECT SUM(CASE WHEN es_atleta THEN 1 ELSE 0 END) as atletas, SUM(CASE WHEN es_coach THEN 1 ELSE 0 END) as coaches FROM dw.dim_miembros")).mappings().first()

        res_genero = db.session.execute(text("""
            SELECT CASE WHEN SUBSTRING(curp, 11, 1) = 'H' THEN 'Hombres' WHEN SUBSTRING(curp, 11, 1) = 'M' THEN 'Mujeres' ELSE 'Otro' END as etiqueta, COUNT(*) as total
            FROM dw.dim_miembros WHERE es_atleta = TRUE GROUP BY etiqueta
        """)).all()

        res_edad = db.session.execute(text("""
            SELECT CASE WHEN edad < 18 THEN 'Infantil' WHEN edad BETWEEN 18 AND 25 THEN 'Juvenil' WHEN edad BETWEEN 26 AND 40 THEN 'Adulto' ELSE 'Veterano' END as etiqueta, COUNT(*) as total
            FROM dw.dim_miembros WHERE es_atleta = TRUE GROUP BY etiqueta ORDER BY total DESC
        """)).all()
        ...
        # 4. Único Return (ESTO ES LO QUE ENVÍA TODO AL HTML)
